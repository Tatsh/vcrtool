...
If any changed files are under `vcrtool/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `vcrtool/`, `tests/`,
...
For example, if a commit contains `vcrtool/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`vcrtool/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `vcrtool/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `vcrtool/` prefix and replace `/` with `.` (like module imports).

- Python file `vcrtool/media.py` → `media:`.
- Multiple files under `vcrtool/commands/` → `commands:`.
- Single command file `vcrtool/commands/admin.py` → `commands.admin:`.
